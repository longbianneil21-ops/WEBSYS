...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .btn-share:hover { border-color: #7c3aed !important; color: #7c3aed; }
    .btn-edit {
      background: #fff; color: var(--text);
      border: 1.5px solid var(--border) !important;
    }
    .btn-edit:hover { border-color: #7c3aed !important; color: #7c3aed; }
...
<div class="app-shell">
  ...
  <aside class="sidebar">
    ...
    <nav class="sidebar-nav" aria-label="Dashboard navigation">
      ...
      <a href="SchoolSched.php" class="nav-item active"><span class="nav-item-icon">📋</span>Class Schedule</a>
  ...
  <main class="main-area">
    ...
    <div class="stat-grid">
      <div class="stat-card blue">
        <div class="stat-info">
          <span class="stat-label">Total Units Enrolled</span>
      ...
      <div class="stat-card green">
        <div class="stat-info">
          ...
          <span class="stat-value large-text" id="semester">—</span>
        </div>
        <div class="stat-icon">📅</div>
      </div>
    </div>
    ...
    <div class="card">
          ...
          <table>
            ...
            <tfoot id="totalFooter">
              <tr class="total-row">
                ...
                <td><span class="total-units-val" id="totalUnitsFooter">0</span></td>
                <td colspan="4"></td>
              </tr>
            </tfoot>
          </table>
      ...
      <div class="action-bar">
        ...
        <button type="button" class="action-btn btn-edit"  style="border:1.5px solid var(--border);">✏️ Edit Enrollment</button>
      </div>
    </div>
  </main>
</div>
...
<script>
  const API_URL = '../../WEBSYS/php/api/class-schedule.php';

  // Load schedule data for current user's section
  async function loadScheduleData() {
    try {
      // Get user's section from localStorage or default
      const userSection = localStorage.getItem('userSection') || 'SBIT-1A';
      
      const response = await fetch(`${API_URL}?action=get&section=${encodeURIComponent(userSection)}`);
      ...
      const totalUnitsFooter = document.getElementById('totalUnitsFooter');
      const totalUnitsHeader = document.getElementById('totalUnits');
      const enrolledCoursesHeader = document.getElementById('enrolledCourses');
      
      tableBody.innerHTML = '';
      let totalUnits = 0;
      
      if (scheduleData.length === 0) {
        tableBody.innerHTML = '<tr><td colspan="8" style="text-align:center; padding:32px; color:var(--text-muted);">No classes scheduled for your section yet. Check back later!</td></tr>';
        totalUnitsFooter.textContent = '0';
        totalUnitsHeader.textContent = '0';
        enrolledCoursesHeader.textContent = '0';
        return;
      }
      
      scheduleData.forEach(course => {
        totalUnits += parseInt(course.units) || 0;
        const row = document.createElement('tr');
        row.innerHTML = `
          <td><span class="course-code">${course.course_code}</span></td>
          <td class="course-name-cell">${course.course_name}</td>
          <td><span class="section-pill">${course.course_section}</span></td>
          <td><span class="units-val">${course.units}</span></td>
          <td class="day-cell">${course.days}</td>
          <td class="time-cell">${course.time}</td>
          <td class="room-cell">${course.room}</td>
          <td class="prof-cell">${course.instructor}</td>
        `;
        tableBody.appendChild(row);
      });
      
      totalUnitsFooter.textContent = totalUnits;
      totalUnitsHeader.textContent = totalUnits;
      enrolledCoursesHeader.textContent = scheduleData.length;
    } catch (error) {
      console.error('Error loading schedule:', error);
      document.getElementById('scheduleTableBody').innerHTML = '<tr><td colspan="8" style="text-align:center; padding:32px; color:var(--text-muted);">Error loading schedule data. Try refreshing the page.</td></tr>';
    }
  }
